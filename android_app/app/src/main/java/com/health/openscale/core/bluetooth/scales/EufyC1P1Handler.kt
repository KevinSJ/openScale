 ...
 */
class EufyC1P1Handler : ScaleDeviceHandler() {

    // --- UUIDs (16-bit under Bluetooth Base UUID) ------------------------------
    private val SVC_FFF0 = uuid16(0xFFF0) //main service
    private val SVC_180F = uuid16(0x180F)

    private val CHR_2A19 = uuid16(0x2A19)
    ...
    override fun onConnected(user: ScaleUser) {
        ...
        setNotifyOn(p.mainService, p.chrNotify)

        // 3) Configure unit/group (matches legacy magic FD 37 ...)
        writeTo(p.mainService, p.chrWrite, buildModeUnitCmd(user))

        // 3) Sync device clock, then wait for ACK "F1 00"
        val clock = buildClockCmd()
        waitAckClock = true
        writeTo(p.mainService, p.chrWrite, clock)
    ...
    override fun onNotification(characteristic: UUID, data: ByteArray, user: ScaleUser) {
        ...
        when (characteristic) {
            ...
            p.chrNotify -> processMeasurements(data, user)
            else -> {
    ...
    private fun parseMeasurementFrame(bytes: ByteArray, user: ScaleUser, isHistoric: Boolean) {
        ...
        val impedanceOhm =
            (((bytes[2].toInt() and 0xFF) shl 8) + (bytes[1].toInt() and 0xFF)) * 0.1f
    ...
    private fun mapUserToLibParams(u: ScaleUser): Pair<Int, Int> {
        ...
        val peopleType = when (u.activityLevel) {
            ...
            ActivityLevel.MILD -> 0
            ActivityLevel.MODERATE -> 1
            ActivityLevel.HEAVY -> 2
            ActivityLevel.EXTREME -> 2
        }
